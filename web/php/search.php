...
<div class="jumbotron text-center" style="padding-top:100px;padding-bottom:100px">
  <div class="container" style="padding-top:100px;">
    ...
    <form name="Search" action="search.php" method="get" enctype="application/x-www-form-urlencoded" class="search" style="width:45%;margin:auto;background:#FFF;padding:10px;border-radius:5px;color:#000000">
      ...
      <input type="submit" class="btn btn-success" value="Buscar" style="width:18%;float:left;padding:5px;margin-left:1%">
      </div>
    </form>
  </div>
<?php 
	...
	include ('usuario.php');
	$urlAccount ='https://twitter.com/'.$_GET["busqueda"];
	$twitter = new TwitterAPIExchange(settings());
	$ObjectResult = json_decode($twitter->setGetfield(createGetField($_GET["busqueda"]))->buildOauth(createUrl(),createRequestMethod())->performRequest(),true);
	$Usuario = new Usuario($ObjectResult);
	if($ObjectResult){
		?>
		<div class="busqueda" style="width:45%;margin:5px auto;padding:10px;background:#FFF;color:#000000;overflow:hidden;border-radius:10px">
			<img class="rounded" style="width:30%;float:left;margin-right:2%" src="<?php echo $ObjectResult["profile_image_url"]; ?>" alt="">
			<div class="information" style="float:left;width:65%;font-size:16px;margin-right:1%;text-align:left">
				<ol style="display:block;list-style:none;">
					<li style="margin-top:3px;margin-bottom:3px;width:100%;"><b>Nombre:</b>&nbsp<?php print $Usuario->getUsername(); ?>.</li>
					<li style="margin-top:3px;margin-bottom:3px;width:100%;"><b>Username:</b>&nbsp@<?php echo $ObjectResult["screen_name"]; ?>.</li>
					<li style="margin-top:3px;margin-bottom:3px;width:100%;"><b>Ubicación:</b>&nbsp<?php echo $ObjectResult["location"]; ?>.</li>
					<li style="margin-top:3px;margin-bottom:3px;width:100%;"><b>Seguidores:</b>&nbsp<?php echo $ObjectResult["followers_count"]; ?>.</li>
					<li style="margin-top:3px;margin-bottom:3px;width:100%;"><b>Cuenta creada:</b>&nbsp<?php echo $ObjectResult["created_at"]; ?></li>

				</ol>
				<a href="<?php echo $urlAccount; ?>"><button class="btn btn-success" style="width:100%">Entrar al perfil</button></a>
			</div>
		</div>
		<?php	
	}else if(empty($ObjectResult)){
		?>"<div style="margin:auto;width:45%" class="alert alert-warning" role="alert">
